"""
Sends synthetic traffic to a running instance of the API:
  1. A batch of "normal" requests matching the training distribution.
  2. A batch of "drifted" requests simulating a market/data shift.
...
def send_batch(url: str, df, label: str, delay: float):
    ...
    for _, row in df.iterrows():
        payload = {col: (int(row[col]) if col == "bedrooms" else float(row[col])) for col in FEATURE_COLUMNS}
        try:
...
def main():
    ...
    normal_df = generate_housing_data(n=args.normal, drift=False)
    send_batch(args.url, normal_df, "normal", args.delay)

    if args.drift > 0:
        print("Sending drifted traffic...")
        drift_df = generate_housing_data(n=args.drift, drift=True)
